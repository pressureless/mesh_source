...
struct heartlib {
        ...
        const int & f)
    {
        ...
        Eigen::Matrix<double, 2, 2> m_r_0;
        m_r_0 << br - (ar).template cast<double>(), cr - (ar).template cast<double>();
        return m_r_0;    
    }
        ...
        const std::vector<Eigen::Matrix<REAL, 2, 1>> & x)
    {
        ...
        // A = ½ |`m_r`(f)|
        REAL A = (1/double(2)) * (m_r(f)).determinant();

        // J = m `m_r`(f)⁻¹
        Eigen::Matrix<REAL, 2, 2> J = m * m_r(f).inverse();
        if((m).determinant() <= 0){
            S_ret = INFINITY;
        }
        else{
