...
ϕ(i, j) = π - atan2(‖`N_1`×`N_1`‖, `N_1`⋅`N_2`) where i, j ∈ V, 
...
struct iheartmesh {
        ...
        const int & j)
    {
        ...
        // `N_1` = N(`f_1`)
        Eigen::Matrix<double, 3, 1> N_2 = N(f_1);

        // `N_2` = N(`f_2`)
        Eigen::Matrix<double, 3, 1> N_1 = N(f_2);
        std::cout<<"N_1: "<<N_1<<std::endl;
        std::cout<<"N_2: "<<N_2<<std::endl;
        std::cout<<"e: "<<e<<std::endl;

        double angle = atan2((e).dot(((N_1).cross(N_2))), (N_1).dot(N_2));   
    ...
    //     // std::cout<<"N_2: "<<N_2<<std::endl;
    //     // std::cout<<"1: "<<((N_1).cross(N_1)).template lpNorm<2>()<<std::endl;
    //     // std::cout<<"2: "<<(N_1).dot(N_2)<<std::endl;
    //     // std::cout<<"3: "<<((N_1).cross(N_1)).template lpNorm<2>()/((N_1).dot(N_2))<<std::endl;
    //     // std::cout<<"atan2: "<<atan2(((N_1).cross(N_2)).template lpNorm<2>(), (N_1).dot(N_2))<<std::endl;
    //     return M_PI - atan2(((N_1).cross(N_2)).template lpNorm<2>(), (N_1).dot(N_2));    
    // }
    double H(
        const int & i)
    {
        ...
        for(int j : this->VertexOneRing(i)){
            ...
            double leng = l(i, j);
            sum_2 += leng * angle;
            std::cout<<"j: "<<j<<", l: "<<leng<<", ϕ: "<<angle<<std::endl<<std::endl;
        }
