...
//         // i,j,k = GetOrientedVertices(f)
//         std::tuple< int, int, int > tuple = GetOrientedVertices(f);
//         int i = std::get<0>(tuple);
//         int j = std::get<1>(tuple);
//         int k = std::get<2>(tuple);
//         if (1 / double(2) * (((x.at(j) - x.at(i))).cross((x.at(k) - x.at(i)))).lpNorm<2>() < 0)
//         {
//             std::cout<<"area is less than 0"<<std::endl;
//         }
//         return 1 / double(2) * (((x.at(j) - x.at(i))).cross((x.at(k) - x.at(i)))).lpNorm<2>();    
//     }
//     double angle(
//         const int & k,
//         const int & j,
...
//         double aa = ((x.at(j) - x.at(k))).dot((x.at(i) - x.at(k))) / double((((x.at(j) - x.at(k))).lpNorm<2>() * ((x.at(i) - x.at(k))).lpNorm<2>()));
//         if (aa < 0)
//         {
//             // std::cout<<"aa :"<<aa<<std::endl;
//         }
//         return acos(((x.at(j) - x.at(k))).dot((x.at(i) - x.at(k))) / double((((x.at(j) - x.at(k))).lpNorm<2>() * ((x.at(i) - x.at(k))).lpNorm<2>())));    
//     }
...
//             sum_0 += area(f, x);
//         }
//         // A = (sum_(f ∈ FaceNeighbors(i)) area(f, x))
//         double A = (sum_0);

//         Eigen::MatrixXd sum_1 = Eigen::MatrixXd::Zero(3, 1);
//         for(int j : VertexOneRing(i)){
//                 // k, l = OppositeVertices(GetEdgeIndex(i,j))
//             std::tuple< int, int > tuple_1 = OppositeVertices(GetEdgeIndex(i, j));
//             int k = std::get<0>(tuple_1);
//             int l = std::get<1>(tuple_1);
//                 // α = angle(k, j, i, x)
//             double α = angle(k, j, i, x);
//                 // β = angle(l, i, j, x)
//             double β = angle(l, i, j, x);
            
//             if ((1/tan(α) + 1/tan(β)) > 0)
//             {
//                 sum_1 += (1/tan(α) + 1/tan(β)) * (x.at(j) - x.at(i));
//                 // std::cout<<"cot weight:"<<(1/tan(α) + 1/tan(β))<<std::endl;
//             }
//         }
//         // K = 1/(4A)(sum_(j ∈ VertexOneRing(i)) (cot(α) + cot(β))(x_j - x_i) 
//     // where k, l = OppositeVertices(GetEdgeIndex(i,j)),
//     // α = angle(k, j, i, x),
//     // β = angle(l, i, j, x) )
//         Eigen::Matrix<double, 3, 1> K = 1 / double((4 * A)) * (sum_1);
//         return x.at(i) - t * K;    
//     }
...
struct iheartla {
        ...
        const std::vector<Eigen::Matrix<double, 3, 1>> & x)
    {
        ...
        int j = std::get<1>(tuple);
        int k = std::get<2>(tuple);
        double area = 1 / double(2) * (((x.at(j) - x.at(i))).cross((x.at(k) - x.at(i)))).lpNorm<2>(); 
        if (area < 1e-8)
        {
            std::cout<<"too small area: "<<area<<std::endl;
            if (area < std::numeric_limits<double>::min())
            {
                std::cout<<"sssssss: "<<area<<std::endl;
            }
        }
        return area;
    }
        ...
        const std::vector<Eigen::Matrix<double, 3, 1>> & x)
    {
        ...
        double aa = ((x.at(oj) - x.at(k))).dot((x.at(oi) - x.at(k))) / double((((x.at(oj) - x.at(k))).lpNorm<2>() * ((x.at(oi) - x.at(k))).lpNorm<2>()));
        if (aa < 0)
        {
            // std::cout<<"aa: "<<aa<<std::endl;
            // std::cout<<"current : ("<<k<<", "<<oj<<", "<<oi<<")"<<std::endl;
            // std::cout<<"k : ("<<x.at(k)(0)<<", "<<x.at(k)(1)<<", "<<x.at(k)(2)<<")"<<std::endl; 
            // std::cout<<"oj : ("<<x.at(oj)(0)<<", "<<x.at(oj)(1)<<", "<<x.at(oj)(2)<<")"<<std::endl; 
            // std::cout<<"oi : ("<<x.at(oi)(0)<<", "<<x.at(oi)(1)<<", "<<x.at(oi)(2)<<")"<<std::endl; 
        }
