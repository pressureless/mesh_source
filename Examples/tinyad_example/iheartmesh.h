...
A_i,j,k = ½ |[x̄_j-x̄_i x̄_k-x̄_i]| where i,j,k ∈ V

S(f, x) = A_abc (‖J‖² + ‖J⁻¹‖²) where f ∈ F, x_i ∈ ℝ^2,
a, b, c = OrientedVertices(f),
J = [x_b-x_a x_c-x_a][x̄_b-x̄_a x̄_c-x̄_a]⁻¹
...
struct iheartmesh {
        ...
        const int & k)
    {
        ...
        A_0 << x̄.at(j) - x̄.at(i), x̄.at(k) - x̄.at(i);
        return (1/double(2)) * (A_0).determinant();    
    }
    double S(
        const int & f,
        const std::vector<Eigen::Matrix<double, 2, 1>> & x)
    {
        ...
        // a, b, c = OrientedVertices(f)
        std::tuple< int, int, int > tuple = OrientedVertices(f);
        ...
        Eigen::Matrix<double, 2, 2> J_0;
        J_0 << x.at(b) - x.at(a), x.at(c) - x.at(a);
        Eigen::Matrix<double, 2, 2> J_1;
        J_1 << x̄.at(b) - x̄.at(a), x̄.at(c) - x̄.at(a);
        std::cout<<"current J_0:"<<J_0<<std::endl;
        std::cout<<"current J_1:"<<J_1<<std::endl;
        // J = [x_b-x_a x_c-x_a][x̄_b-x̄_a x̄_c-x̄_a]⁻¹
        Eigen::Matrix<double, 2, 2> J = J_0 * J_1.inverse();
        return A(a, b, c) * (pow((J).norm(), 2) + pow((J.inverse()).norm(), 2));    
    }
    double Energy(
        const std::vector<Eigen::Matrix<double, 2, 1>> & x)
    {
    ...
    iheartmesh(
        ...
        const std::vector<Eigen::Matrix<double, 2, 1>> & x̄)
    :
    _Neighborhoods(M)
    {
        std::tuple< std::vector<int >, std::vector<int >, std::vector<int > > rhs = M.MeshSets();
