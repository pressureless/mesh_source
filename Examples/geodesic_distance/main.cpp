...
struct iheartla {
    ...
    // q(0, 0) = x1.dot(x1);  // X0 dot_product X0
    // q(0, 1) = x1.dot(x2); 
    // q(1, 0) = x2.dot(x1); 
    // q(1, 1) = x2.dot(x2); 
    ...
    // std::cout<<"QQ is"<<QQ<<std::endl;
        ...
        double aa = pow(((double)((num1.transpose()).cast<double>() * Q * t)), 2) - (double)((num1.transpose()).cast<double>() * Q * (num1).cast<double>()) * ((double)(t.transpose() * Q * t) - 1);

        if (aa < 0)
        {
            // std::cout<<"aa: "<<aa<<std::endl;
            // std::cout<<"power: "<<pow(((double)((num1.transpose()).cast<double>() * Q * t)), 2)<<std::endl;
            // std::cout<<"after: "<<(double)((num1.transpose()).cast<double>() * Q * (num1).cast<double>()) * ((double)(t.transpose() * Q * t) - 1)<<std::endl;
            // std::cout<<"e1:"<<(x.at(v1) - x.at(v0)).lpNorm<2>()<<", e2:"<<(x.at(v2) - x.at(v0)).lpNorm<2>()<<", e3:"<<(x.at(v1) - x.at(v2)).lpNorm<2>()<<std::endl;
        }
        

        // n = XQ(t-`1`)
        Eigen::Matrix<double, 3, 1> n = X * Q * (t - p * (num1).cast<double>());
        ...
        if((s(1-1, 1-1) < 0) && (s(2-1, 1-1) < 0)){
            UpdateStep_ret = p;
            // std::cout<<"p  UpdateStep_ret: "<<UpdateStep_ret<<std::endl;
        }
        else{
            UpdateStep_ret = Min(d.at(v1) + (x1).lpNorm<2>(), d.at(v2) + (x2).lpNorm<2>());
            // std::cout<<"d.at(v1) + (x1).lpNorm<2>(): "<<d.at(v1) + (x1).lpNorm<2>()<<std::endl;
            // std::cout<<"d.at(v2) + (x2).lpNorm<2>(): "<<d.at(v2) + (x2).lpNorm<2>()<<std::endl;
        }
        if (UpdateStep_ret == 0)
        {
        ...
        // std::cout<<"UpdateStep_ret: "<<UpdateStep_ret<<std::endl;
...
int main(int argc, const char * argv[]) {
    ...
    // int ret = ihla.UpdateStep(0, 1, 2, distance);
