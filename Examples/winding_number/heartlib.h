...
Ω_f(𝐩) = 2 atan2(|[𝐚 𝐛 𝐜]|, (abc+(𝐚⋅𝐛)c+(𝐛⋅𝐜)a+(𝐜⋅𝐚)b)) where f ∈ F, 𝐩 ∈ ℝ³,
𝐚 = x_i - 𝐩,
𝐛 = x_j - 𝐩,
𝐜 = x_k - 𝐩,
a = ‖𝐚‖,
...
w(𝐩) = 1/(4π) ∑_(f ∈ F) Ω_f(𝐩) where 𝐩 ∈ ℝ³
...
struct heartlib {
        ...
        const Eigen::Matrix<REAL, 3, 1> & 𝐩)
    {
        ...
        // 𝐚 = x_i - 𝐩
        Eigen::Matrix<REAL, 3, 1> 𝐚 = this->x.at(i) - 𝐩;

        // 𝐛 = x_j - 𝐩
        Eigen::Matrix<REAL, 3, 1> 𝐛 = this->x.at(j) - 𝐩;

        // 𝐜 = x_k - 𝐩
        Eigen::Matrix<REAL, 3, 1> 𝐜 = this->x.at(k) - 𝐩;

        // a = ‖𝐚‖
        REAL a = (𝐚).template lpNorm<2>();

        // b = ‖𝐛‖
        REAL b = (𝐛).template lpNorm<2>();

        // c = ‖𝐜‖
        REAL c = (𝐜).template lpNorm<2>();
        Eigen::Matrix<REAL, 3, 3> Ω_0;
        Ω_0 << 𝐚, 𝐛, 𝐜;
        return 2 * atan2((Ω_0).determinant(), (a * b * c + ((𝐚).dot(𝐛)) * c + ((𝐛).dot(𝐜)) * a + ((𝐜).dot(𝐚)) * b));    
    }
    template<typename REAL>
    REAL w(
        const Eigen::Matrix<REAL, 3, 1> & 𝐩)
